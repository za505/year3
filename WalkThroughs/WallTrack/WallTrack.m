...
%File Parameters
basename='01142022_Exp1';%Name to save the file to
skp=[];%frames skipped
K=[]; %frames to omit
dirname=['/Users/zarina/Downloads/NYU/Year3_2022_Spring/01142022_analysis/'  basename '_colony3/' basename '_TADA/' basename '_erased'];%directory where the imagestack is saved
imdir=['/Users/zarina/Downloads/NYU/Year3_2022_Spring/01142022_analysis/'  basename '_colony3/' basename '_mNeonGreen/' basename '_aligned'];%directory where the imagestack is saved
savedir=['/Users/zarina/Downloads/NYU/Year3_2022_Spring/01142022_analysis/'  basename '_colony3/' basename '_TADA/' basename '_figures'];
%metaname=['/Matlab Ready/'  basename '.txt'];%full pathname of the metadata file

%Tracking Parameters
ll=0.35;%parameter used to enhance contrast of image before performing edge detection % default=0.35
ppix=0.3;%percent of pixels to saturate during normalization default=0.6, %0.26 FOR mNeonGreen (the lower the value, the dimmer the fluor will be)
cutL=0.2;%low threshold for edge detector default=0.7
...
multiscale=0; 
tscale=60;%Time between frames %Default=30
...
%Other Parameters
recrunch=1;%whether or not to re-plot a file using new analysis parameters
plotcells=0;%whether or not to visualize the tracking
troubleshoot=0; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if recrunch==1
    cd(savedir);
    load([basename '_colony3_WT'],'-regexp','^(?!lscale$|tscale$|minA$|maxI$|minW$|maxW$|minL$|maxD$).')
else

%save GFP image directory
cd(imdir);
imfiles=dir('*.tif');
...
%     % Initialize video
% myVideo = VideoWriter('mNeonGreen_tracking'); %open video file
...
for t=1:T
    ...
    if ~isempty(K) & ismember(t, K)
        ...
        %update concatenated variables
        allcentroids=[allcentroids;centroids];
        tstamp=[tstamp;ones(nc(t-1),1)*t];
        cellnum=[cellnum;(1:nc(t-1))'];
        

        %overlay cell boundaries with GFP image to make sure overlap is not
        %totally off
        %if plotcells==1
            cd(imdir);
...
%     [~,mpos]=min(abs(pcsic-ppix/100));
%     maxintensity=imbin(end-mpos);
%     im2=imadjust(im,[0 maxintensity/65535],[]);
%     imshow(im2)
%     flag=input('Is this adjustment okay? 1=yes, 0=no: ')
%         if flag==1
%             ppix=ppix;
%         else
%             ppix=ppix+0.1;
...
ws=labelmatrix(cc);
    ...
    stats=regionprops(cc,ws,'Area');
    idx=find([stats.Area]<3000 & [stats.Area]>minA);
    %for n=1:length(unique(ws)), bwmx=zeros(size(ws));, bwmx(ws==n)=1;,
    %imshow(bwmx), pause, close, end
    ws=ismember(ws, idx);
...
delind=[];
for i=1:ncells
    if length(nonzeros(lcell(i,:)))<=2 | sum(isnan(lcell(i,:)))>=T/2
        delind=[delind;i];
    end
end
lcell(delind,:)=[];
...
%throw away cells that lost all fluor. too early
delind=[2, 4, 6, 10, 11, 12, 16, 22];
lcell(delind,:)=[];
wcell(delind,:)=[];
B(delind,:)=[];
pixels(delind,:)=[];%edit for color decode DF
...
ylabel('length (\mum)')
fig2pretty
saveas(gcf, [basename 'lTrace.fig'])
saveas(gcf, [basename 'lTrace.png'])
