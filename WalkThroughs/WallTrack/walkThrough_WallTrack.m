...
% Purpose: to walk through WallTrack.m line by line. Using live script is
% just painful, so I opted for this option instead. 

% WallTrack.m
% Purpose: this code tracks cell growth from stacks of fluorescent images where the cell periphery has been stained. it also records the intensity of a tracer dye taken in a different channel simultaneously. In this example, I do not have an image stack with tracer dye, will the code still work?
% 
% Instructions:
% 1)If there is significant drift in your movie, register the images before you use this code or else it may have trouble tracking cells frame to frame
% 2)Save the image sequence in a folder by itself.
% 3)Modify the User Input section below accordingly.
% 
% Output Format:
% The data is saved as a .mat file as 'basename_WT' in the working directory, where basename is a input string chosen by the user.  The main
...
% lcell:  ncells x T matrix containing the length of the cells in the movie. each row is an individual cell and each column is a different time point
...
% vav:    1 x T-1 vector containing the average growth rate of the cells, averaged across cells as a function of time.
% B:      ncells x T cell array containing the x and y coordinates of the  contours for each cell.
% 
% Calls on:
% norm16bit.m = normalizes the intensity of a 16-bit image, saturating a certain percentage of pixels and the min and max values
% polefinder.m = I still am not totally sure how this works, but it finds the poles of the cells
% cellcurvature.m = calculates the curvature of the cell contour and averages over the neighboring points
% metadata.m = extracts time from micromanager metadata, I don't usually use this, but I've often wondered how I could
% extrema.m = gets the global extreme points from a time series
% fig2pretty.m = make figures prettier
% ciplot.m = plots a shaded region on a graph to specify a confidence interval
...
skp=[];%frames to skip
dirname=['/Users/zarina/Downloads/NYU/Year3_2021_Fall/PlasmolysisTrack/' basename '/' basename '_TADA/' basename '_erased'];%directory where the imagestack is saved
imdir=['/Users/zarina/Downloads/NYU/Year3_2021_Fall/PlasmolysisTrack/' basename '/' basename '_GFP/' basename '_aligned'];%directory where the imagestack is saved
savedir=['/Users/zarina/Downloads/NYU/Year3_2021_Fall/PlasmolysisTrack/' basename '/' basename '_TADA/' basename '_figures'];
...
tscale=30;%Time between frames %Default=30 %for this data set, this is technically not true, but it also doesn't matter for it
minA=100;%Minimum cell area %Default=20
...
troubleshoot=1; %this is a parameter I added that is not in the original code
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Next, let's see what the code does to the image at a single time point. 
for t=1:T 
    %t=1 %here, I am setting t=1, so I will be going through line by line instead of just running the loop
    
    %load images
    imagename=directory(t).name;
    im=imread(imagename);
    imraw=im;
    %imraw=imcomplement(im); %why was this commented out in the original
    %file? 
    [imm,imn]=size(im);
    %figure(1), imshow(im)
    %figure(2), imshow(im, []) %sets the min pixel value as black and max pixel value as white
    
    %check image histogram, if necessary 
    %figure(3),imhist(im)

    %normalize images
    [imcounts,imbin]=imhist(im); %imbin = given pixel value, imcount = how many of a given pixel value are in the image in order of the pixel values from lowest to highest
    csic=cumsum(fliplr(imcounts')); %transpose, flip so counts for the highest pixel values are first, then take the cumulative sum
    pcsic=csic/sum(imcounts); %the cumulative sum at each given time point is what proportion of the total
    
    %between 0.9 and 0.4, the mpos value has a difference of 5, (220 and
    %215, respectively). but this small different in mpos translates to a
    %bigger difference in maxintensity, 8995 and 10280, respectively. Which
    %maps to 0.1373 and 0.1569. ppix=0.4 has a greater range of lower
    %values, I think. 
    [~,mpos]=min(abs(pcsic-ppix/100)); %why divide by 100? It's already a proportion?
    ...
    %enhance contrast
    im=imadjust(im,[ll 1],[]); %ll=the lower threshold that the image will be mapped to, shifting the histogram back to the left
    %figure,imshow(im),pause
    
    %perform edge detection
    im=medfilt2(im); %I don't see much of a difference
    bw=edge(im,'canny',cutL,cutH); %the edge detection is a bit cleaner post median filtering, but flaws are enhanced
    bw=imcomplement(bw);
    ...
    %watershed image, what is the point of this step?
%     im(bw2==1)=-Inf;
%     ws=watershed(im);
      ws=bw2;
