function celldata = cload(varargin)
...
param.mincellperim = 30;
param.maxcellperim = 500;
param.minarea = 50;
param.maxarea = Inf;
param.npoints = 200;
param.optmid = true;
param.filterwidth = true;
param.contourlevel = 100;
param.channel = 1;
param.wdfilter = true;
param.minthick = 0;
param.maxthick = Inf;
    ...
    function paramset(startpoint)
        ...
        elseif ischar(varargin{startpoint})==0
            ...
            if isfield(param, lower(varargin{startpoint}))
                ...
                elseif isa(logical(varargin{startpoint+1}),class(param.(lower(varargin{startpoint}))))
                    param.(lower(varargin{startpoint})) = logical(varargin{startpoint+1});
                    startpoint = startpoint +1;
                else
...
% Filter by width (manual limits)
if isfield(celldata.cells, 'thickness')
maxwidth = max(reshape([celldata.cells.thickness],param.npoints,celldata.CellCount));
	celldata.cells(maxwidth > param.maxthick | maxwidth < param.minthick) = [];
	clear maxwidth
end
% Filter by thickness (statistically)
if param.filterwidth && isfield(celldata.cells, 'thickness')
	maxwidth = max(reshape([celldata.cells.thickness],param.npoints,length(celldata.cells)));
...
if del == 0
    warning('Can not subdivide a curve with length zero')
	points = [];
	return
end
